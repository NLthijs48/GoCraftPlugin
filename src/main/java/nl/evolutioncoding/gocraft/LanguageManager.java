...
public class LanguageManager {
	...
	/**
	 * Constructor
	 * @param plugin The GoCraft plugin
	 */
	...
	public void saveDefaults() {
		...
		File langFolder;
		langFolder = new File(plugin.getDataFolder() + File.separator + "lang");
		...
		for(String language : languages) {
			langFile = new File(plugin.getDataFolder() + File.separator + "lang" + File.separator + language + ".yml");
			try(
					InputStream input = plugin.getResource("lang" + "/" + language + ".yml");
					OutputStream output = new FileOutputStream(langFile)
			) {
				if(input == null) {
					plugin.getLogger().warning("Could not save default language to the '" + "lang" + "' folder: " + language + ".yml");
					continue;
				}
				int read;
	...
	public void loadLanguage() {
		...
		File standard = new File(plugin.getDataFolder() + File.separator + "lang" + "/" + languages[0] + ".yml");
		try(
